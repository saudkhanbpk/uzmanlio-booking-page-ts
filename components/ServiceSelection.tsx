...
export const ServiceSelection: React.FC = () => {
  ...
  const [serviceType, setServiceType] = React.useState<"bireysel" | "grup" | "paket">("bireysel");
  ...
  // ✅ Handle tab change
  const handleServiceTypeChange = (type: "bireysel" | "grup" | "paket") => {
    setServiceType(type);
    resetBookingState();
  };

  // ✅ Handle selection
  const handleSelect = (id: string, type: "service" | "package") => {
    if (type === "service") {
  ...
  function finalPrice(item) {
    ...
    return Math.round(discounted); // Rounded for clean display
  }
  // ✅ Render cards
  const renderCards = (list: any[], type: "service" | "package") => (
            ...
            >
              <div className="flex justify-between items-start">
                <div className={type === "package" ? "space-y-2" : ""}>
                  ...
                  {type === "service" ? (
                    <>
                      {/* Indiviual service details */}
                      {(item.meetingType === "1-1" || item.meetingType === "bireysel" || item.meetingType === "face-to-face") && (
