...
import { CouponResponse } from "@/components/types/booking.types";
...
const applyCoupon = async () => {
  ...
  try {
    const response = await axios.post<CouponResponse>(
      `/api/booking/${customerId}/coupon/${code}`
    );
    ...
    // ✅ Extract useful error message from backend or fallback
    const backendError =
  ...
  const calculatePricing = () => {
    ...
    const total = Math.max(subtotal - discount, 0); // prevent negative totals
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`✅ Rezervasyon başarıyla oluşturuldu(${result.bookingId || customerId})! for ${profile?.information.name} ${profile?.information.surname}`);
